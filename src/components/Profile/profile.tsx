...
const ProfileHeading: FC<ProfileHeadingProps> = ({
  ...
  const { data: viewer = viewerSession, status: status = viewerStatus } =
    useSession();
  const imageHandler = viewer?.user?.image
    ? viewer.user.image.includes("pbs.twimg.com")
      ? viewer.user.image.split("normal.jpg")[0].concat("400x400.jpg")
      : viewer.user.image.includes("lh3.googleusercontent.com")
      ? viewer.user.image.split("s96-c")[0].concat("s400-c")
      : viewer.user.image.includes("avatars.githubusercontent.com")
      ? viewer.user.image.concat("&w=400&h=400&q=100")
      : "https://c.tenor.com/OeBiYk-IfZEAAAAC/archer-coroca.gif"
    : "https://c.tenor.com/OeBiYk-IfZEAAAAC/archer-coroca.gif";
  return (
    <div className='font-interVar'>
      {status === "loading" ? (
        ...
        <>
          <div className='relative bg-gradient-to-br from-gray-700 via-slate-800 to-gray-900 flex h-[128px]'>
            {/* <Image
              quality={90}
              placeholder='blur'
              objectFit='cover'
              ...
              alt={`${viewer?.user?.name}'s cover image`}
          ...
          <div className='max-w-5xl mx-auto px-4 sm:px-6 lg:px-8'>
            <div className='-mt-12 sm:-mt-16 sm:flex sm:items-end sm:space-x-5'>
              <div className='z-100 sm:h-32 rounded-full sm:w-32'>
                {viewer?.user?.image ? (
                  <Image
                    priority={true}
                    className=' z-100 object-cover rounded-full'
                    src={imageHandler}
                    layout='responsive'
                    objectFit={"cover"}
                    width={400}
              ...
              <div className='mt-6 sm:flex-1 sm:min-w-0 sm:flex sm:items-center sm:justify-end sm:space-x-6 sm:pb-1'>
                <div className='sm:hidden relative md:block min-w-0 flex-1 -top-20 '>
                  <h1 className='text-[1.25rem] font-bold text-stone-200 truncate absolute -top-22'>
            ...
            <div className='hidden sm:block md:hidden mt-6 min-w-0 flex-1'>
              <h1 className='text-2xl font-bold text-white truncate'>
                {viewer?.user?.name ? viewer.user.name : "Name Null"}
